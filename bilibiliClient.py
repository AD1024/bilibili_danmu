...
class bilibiliClient():
		...
		while self.connected == True:
			...
			if num2 != 0:
				...
				if num == 0 or num == 1 or num == 2:
					tmp = await self._reader.read(4)
					num3, = unpack('!I', tmp)
					print ('\033[1;31;43m 当前房间人数为 %s \033[0m' % num3)
	...
	def parseDanMu(self, messages):
		...
		if cmd == 'DANMU_MSG':
			...
			if ban.blackList.__contains__(commentUser):
				return
			isAdmin = dic['info'][2][2] == 1
			isVIP = dic['info'][2][3] == 1
			if isAdmin:
				commentUser = '\033[1;0;46m [房管]\033[0m ' + commentUser
			if isVIP:
				commentUser = '\033[1;0;41m [VIP] \033[0m ' + commentUser
			try:
		...
		if cmd == 'SEND_GIFT' and config.TURN_GIFT == 1:
			...
			GiftAction = dic['data']['action']
			try:
				# print()
				print('\033[1;33m ' + GiftUser + ' \033[0m' + ' ' + GiftAction + ' ' + str(GiftNum) + ' 个' + '\033[1;31m ' + GiftName + ' \033[0m')
			except:
				pass
			return
		if cmd == 'WELCOME' and config.TURN_WELCOME == 1:
			...
			if ban.blackList.__contains__(commentUser):
				return;
			try:
				commentUser = '\033[1;31;43m ' + commentUser + '\033[0m!'
